...
pnpm client:generate  # Fresh OpenAPI export -> Orval -> @ifs/api-client (JS + types)
```

`client:generate` includes `openapi:generate`. Neither generation command needs a running API or database connection. Import the client from `@ifs/api-client` in `prototype`; see [client usage](api-client/README.md) and [backend documentation](prototype-api/README.md#swagger-and-openapi).
...
The type and description are required. Scope is optional, but when provided must be a workspace package name: `ifs-standards`, `prototype`, `prototype-api`, or `@ifs/api-client`. Omit the scope for repository-wide changes. When adding or renaming a workspace package, update `scope-enum` in `commitlint.config.mjs` and this list.
